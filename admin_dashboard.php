...
.wrapper{max-width:1300px;margin:30px auto;display:flex;gap:20px}
/* Sidebar */
.sidebar{
    width:260px;background:#8b2e20;color:white;border-radius:20px;
    padding:30px 20px;display:flex;flex-direction:column;align-items:center
}
.sidebar img{width:100px;height:100px;border-radius:50%;border:4px solid #fff;margin-bottom:15px}
.sidebar h2{margin-bottom:30px}
.sidebar a{
    color:white;text-decoration:none;margin:10px 0;width:100%;
    text-align:center;padding:10px;border-radius:8px
}
.sidebar a:hover{background:rgba(255,255,255,.2)}
.logout{margin-top:auto;background:#c14a1b}
/* Content */
.content{flex:1;padding:30px}
h1{color:#7b1f1f;margin-bottom:20px}
/* Cards */
.cards{display:flex;gap:20px;flex-wrap:wrap;margin-bottom:40px}
.card{
    flex:1 1 200px;background:#d49677;border-radius:20px;
...
th{background:#c14a1b;color:white}
...
.onway{background:#f1c40f;color:white}
.stock-btn{background:#c14a1b;color:white;margin:0 3px}
.admin-menu-btn{background:#3498db;color:white;margin:0 3px}
.admin-menu-btn.delete{background:#e74c3c;color:white;margin:0 3px}
/* Admin Menu Grid */
.menu-grid{
    display:grid;
...
@media(max-width:900px){
    ...
    .sidebar h2,.sidebar img{display:none}
}
</style>
...
<h1>Dashboard</h1>
<div class="cards">
    <div class="card"><h4>Products</h4><p><?=getTotal($conn,'menu')?></p></div>
    <div class="card"><h4>Users</h4><p><?=getTotal($conn,'users')?></p></div>
    <div class="card"><h4>Pending Orders</h4><p><?=getCount($conn,'pending')?></p></div>
    <div class="card"><h4>Out for Delivery</h4><p><?=getCount($conn,'on way')?></p></div>
    <div class="card"><h4>Delivered Orders</h4><p><?=getCount($conn,'delivered')?></p></div>
</div>
...
<th>Status</th><th>Items</th><th>Action</th>
</tr>
...
<button value="cancel" class="cancel" onclick="orderAction(<?=$o['order_id']?>,this)">Cancel</button>
<button value="on way" class="onway" onclick="orderAction(<?=$o['order_id']?>,this)">On Way</button>
<?php endif; ?>
</td>
</tr>
<?php endforeach; ?>
...
<div class="menu-card">
    ...
    <div class="admin-actions">
        <button class="admin-menu-btn" onclick="updateMenuStock(<?= $m['menu_id'] ?>,'minus')">−</button>
        <span id="menu-stock-<?= $m['menu_id'] ?>"><?= $m['stock'] ?></span>
        <button class="admin-menu-btn" onclick="updateMenuStock(<?= $m['menu_id'] ?>,'add')">+</button>
        <button class="admin-menu-btn delete" onclick="if(confirm('Delete this item?')) window.location.href='admin_delete_menu.php?id=<?= $m['menu_id'] ?>'">Delete</button>
    </div>
</div>
<?php endforeach; ?>
</div>
